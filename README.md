...
```
L7  PRESENTATION    safety orange + construction blue (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      recruitment-ai-mcp, resume-parser-ai-mcp, lead-scoring-ai-mcp, muckaway-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: UK haulage, grab-lorry fleet, council permits, MCIL/MOL, drivers
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "grabhire"
...
```bash
git clone https://github.com/CSOAI-ORG/grabhire-hive
cd grabhire-hive
...
`https://grabhire.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Marketplace fees + £99-499/mo per rental co.
